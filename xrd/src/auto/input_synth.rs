...
impl InputSynth {
    ...
    pub fn move_cursor(&self, window: &impl IsA<Window>, controller_pose: &mut graphene::Matrix, intersection: &mut graphene::Point3D) {
        unsafe {
            ffi::xrd_input_synth_move_cursor(self.to_glib_none().0, window.as_ref().to_glib_none().0, controller_pose.to_glib_none_mut().0, intersection.to_glib_none_mut().0);
        }
    }
